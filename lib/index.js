...
// this is only necessary if the issuer is running locally
// in development. In production rejectUnauthorized should
// never be false.
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const {credential} = keyValueStore;
    ...
    const postBody = {credential};
    // this calls on the issuer to sign the credential with a proof
    const {data: presentationFromIssuer} = await apiInstance.post(
      issuerEndPoint, postBody);
    presentationFromIssuer.holder = holder;
